      ...
      <TabsList className="grid h-auto w-full grid-cols-3 gap-1 rounded-none border-b border-gray-200 bg-transparent p-0 lg:grid-cols-6">
        {tabs.map((tab, i) => (
          <TabsTrigger
            key={i}
            value={String(i)}
            className="rounded-none border-b-2 border-transparent bg-transparent px-4 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 data-[state=active]:border-meliusly-primary data-[state=active]:bg-transparent data-[state=active]:text-meliusly-primary"
          >
            {tab.title}
          </TabsTrigger>
        ))}
