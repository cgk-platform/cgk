...
interface ProductCardProps {
  ...
  /** Custom class name */
  className?: string
  /** Product rating from reviews metafield */
    ...
    >
      ...
      >
        ...
        <div className="absolute left-2 bottom-2 flex flex-col-reverse gap-1">
          {hasSalePrice && (
            <span className="rounded-full bg-cgk-gold px-2.5 py-0.5 text-xs font-semibold text-white">
              Sale
            </span>
          )}
          {!availableForSale && (
            <span className="rounded-full bg-gray-900/80 px-2.5 py-0.5 text-xs font-semibold text-white">
              Sold Out
            </span>
          )}
        </div>
