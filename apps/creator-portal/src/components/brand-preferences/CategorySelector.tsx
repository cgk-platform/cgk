...
import { BRAND_CATEGORIES } from '@/lib/brand-preferences/constants'
import type { BrandCategory } from '@/lib/types'

interface CategorySelectorProps {
  selected: BrandCategory[]
  onChange: (categories: BrandCategory[]) => void
  disabled?: boolean
}

/**
 * CategorySelector - Multi-select grid of brand categories
 *
 * Features:
      ...
      {BRAND_CATEGORIES.map((category) => {
        ...
        return (
          <button
            ...
            disabled={disabled}
            className={cn(
              'group relative flex items-center gap-2.5 rounded-lg border px-3 py-2.5 text-left text-sm transition-all',
 ...
 */
function CategoryIcon({ name, className }: { name: string; className?: string }): React.JSX.Element {
  // Map icon names to simple SVG icons
  const icons: Record<string, React.JSX.Element> = {
    ...
    car: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
        <path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.5 2.8C1.4 11.3 1 12.2 1 13v3c0 .6.4 1 1 1h2" /><circle cx="7" cy="17" r="2" /><path d="M9 17h6" /><circle cx="17" cy="17" r="2" />
      </svg>
    ),
    paw: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
