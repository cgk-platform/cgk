...
import { AlertCircle, CheckCircle2, Clock, FileText, Mail, Send, XCircle } from 'lucide-react'
...
export default async function EsignQueuePage({ searchParams }: PageProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">E-Sign Document Queue</h1>
          <p className="text-sm text-muted-foreground">
            Monitor document signing request and reminder emails
          </p>
        </div>
        <Link
          href="/admin/communications/queues/esign?status=failed"
...
async function StatsCards() {
  ...
  const stats = await getEmailQueueStats(tenantSlug, 'esign')
  ...
  const cards = [
    ...
    { title: 'Docs Sent', value: stats.total, icon: FileText, color: 'text-purple-600', bgColor: 'bg-purple-50' },
  ]
...
function FiltersBar({ status }: { status: QueueStatus | undefined }) {
  const basePath = '/admin/communications/queues/esign'
...
async function QueueLoader({
  ...
  const { rows, totalCount } = await getEmailQueueEntries(tenantSlug, 'esign', {
    status,
    page,
    limit,
  })
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-16">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {status ? `No ${status} emails found.` : 'E-sign email queue entries will appear here.'}
          </p>
        </CardContent>
      </Card>
    )
  ...
  return (
    <div className="space-y-4">
      <QueueTable entries={entries} queueType="esign" />
      <Pagination
        page={page}
        totalPages={totalPages}
        totalCount={totalCount}
        limit={limit}
        basePath="/admin/communications/queues/esign"
