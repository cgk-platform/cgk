...
export default async function SubscriptionQueuePage({ searchParams }: PageProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Subscription Email Queue</h1>
          <p className="text-sm text-muted-foreground">
            Monitor subscription lifecycle emails (welcome, renewals, cancellations)
          </p>
        </div>
        <Link
          href="/admin/communications/queues/subscription?status=failed"
...
async function StatsCards() {
  ...
  const stats = await getEmailQueueStats(tenantSlug, 'subscription')
...
function FiltersBar({ status }: { status: QueueStatus | undefined }) {
  const basePath = '/admin/communications/queues/subscription'
...
async function QueueLoader({
  ...
  const { rows, totalCount } = await getEmailQueueEntries(tenantSlug, 'subscription', {
    status,
    page,
    limit,
  })
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-16">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {status ? `No ${status} emails found.` : 'Subscription email queue entries will appear here.'}
          </p>
        </CardContent>
      </Card>
    )
  ...
  return (
    <div className="space-y-4">
      <QueueTable entries={entries} queueType="subscription" />
      <Pagination
        page={page}
        totalPages={totalPages}
        totalCount={totalCount}
        limit={limit}
        basePath="/admin/communications/queues/subscription"
