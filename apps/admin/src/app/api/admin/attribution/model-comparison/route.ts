...
import { getModelComparisonData } from '@/lib/attribution'
import type { AttributionWindow } from '@/lib/attribution'

const CACHE_KEY_PREFIX = 'attribution-model-comparison'
const CACHE_TTL = 300
...
export async function GET(request: Request) {
  ...
  const models = await withTenant(tenantSlug, () =>
    getModelComparisonData(tenantId, window, startDate, endDate)
  )
