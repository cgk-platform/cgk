/**
 * Environment variable validation for Admin Portal
 *
...
const OPTIONAL_ENV_VARS = ['SESSION_SECRET', 'STRIPE_SECRET_KEY'] as const
...
if (process.env.NODE_ENV === 'development') {
  ...
  if (result.warnings.length > 0) {
    console.warn(`[ADMIN] Missing optional env vars: ${result.warnings.join(', ')}`)
  }
}
