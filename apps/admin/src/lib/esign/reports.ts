...
export async function getReportData(
  ...
  return withTenant(tenantSlug, async () => {
    const templateCondition = options.templateId
      ? sql`AND template_id = ${options.templateId}`
      : sql``
    ...
    const templatesResult = await sql`
      ...
      FROM esign_documents d
      LEFT JOIN esign_templates t ON t.id = d.template_id
      WHERE d.created_at >= ${options.startDate}
        AND d.created_at <= ${options.endDate}
...
export async function exportReportCsv(
  ...
  return withTenant(tenantSlug, async () => {
    const templateCondition = options.templateId
      ? sql`AND d.template_id = ${options.templateId}`
      : sql``
