...
export function ProductSelectorGuide() {
  ...
  return (
    <div className="mb-8 rounded-lg border border-gray-200 bg-gray-50 p-6">
      <h2 className="font-manrope mb-4 text-xl font-semibold text-[#161f2b]">
        ...
        {guideOptions.map((option, i) => {
          ...
          return (
            <a
              key={i}
              href={option.link}
              className="flex flex-col items-center rounded-lg border border-gray-200 bg-white p-4 text-center transition-all hover:border-[#0268a0] hover:shadow-md"
            >
              <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[#0268a0]/10">
                <Icon className="h-6 w-6 text-[#0268a0]" />
              </div>
              <h3 className="mb-2 text-sm font-semibold text-[#161f2b]">{option.title}</h3>
