...
-- Migration: 037_gift_cards.sql
-- Description: Tables for gift card products, transactions, emails, and settings
...
CREATE INDEX IF NOT EXISTS idx_gift_card_products_variant_numeric ON gift_card_products(variant_id_numeric);
...
CREATE TABLE IF NOT EXISTS gift_card_transactions (
  ...
  customer_name TEXT,
  gift_card_product_id TEXT REFERENCES gift_card_products(id) ON DELETE SET NULL,
...
-- Unique constraint to prevent duplicate credits for same order+variant
CREATE UNIQUE INDEX IF NOT EXISTS idx_gift_card_transactions_unique
  ON gift_card_transactions(shopify_order_id, COALESCE(gift_card_variant_id, ''));
...
CREATE INDEX IF NOT EXISTS idx_gift_card_transactions_source ON gift_card_transactions(source);

-- Gift Card Emails
-- Queue for gift card notification emails
CREATE TABLE IF NOT EXISTS gift_card_emails (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  transaction_id TEXT NOT NULL REFERENCES gift_card_transactions(id) ON DELETE CASCADE,
...
INSERT INTO gift_card_settings (id) VALUES ('default') ON CONFLICT DO NOTHING;
